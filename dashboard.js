...
function attachEventListeners() {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
    // NEW: Check user session and refresh token on page load
    const isSessionValid = await refreshSessionToken();
    if (!isSessionValid) {
        return; // Stop execution if the session is not valid
    }
...
async function refreshSessionToken() {
    ...
    if (currentUser) {
        try {
            // This is the core function to check and extend the session.
            ...
            // The session is invalid, so let's log the user out and redirect.
            await Parse.User.logOut();
        ...
        // No user is logged in, so redirect them.
